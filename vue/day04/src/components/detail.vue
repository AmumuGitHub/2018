<template>
  <div class="detail">
    ...
    <div class="content">
      <div class="header clear"><h2><img width="50" :src="detail.author_face" alt=""/></h2><p>作者名字</p></div>
      <div class="cont">
        <h3>{{detail.title}}</h3>
        <div class="time"><p>{{detail.time | date}}<span><img src="src/assets/img/zan.png" alt=""/></span></p></div>
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    ...
    axios({
      url:'/data/article_'+to.query.dataName+'.data',
    }).then(
      res => {
        // setTimeout(()=>{
          // console.log(res.data);
          // this.detail=res.data[this.$route.params.id-1]
          next(_this=>_this.detail=res.data[to.params.id-1]);//进入路由之后，调用者个函数，传递组件实例，用_this接受
        // },2000);
      }
    )
  }
  /* created(){
    ...
    this.$http({
      url:'/data/article_'+this.$route.query.dataName+'.data',
...
html,body{ overflow-x: hidden; }
    ...
    .nav ul li{width:0.29rem;height:0.31rem; background:url(/src/assets/img/history.png) no-repeat 0 0; background-size:100%; margin:0 0 0 0.38rem;}
    ...
    .say i{width:0.36rem;height:0.26rem; background:url(/src/assets/img/say.png) no-repeat 0 0; background-size:100%; float:left; margin-left:0.7rem; margin-top:0.13rem;}
    ...
    .zan i{width:0.36rem;height:0.28rem; background:url(/src/assets/img/zan1.png) no-repeat 0 0; background-size:100%; float:left; margin-left:0.54rem; margin-top:0.13rem;}
